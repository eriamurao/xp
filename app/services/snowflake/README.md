...
There is a private `machine_id` helper that parses a trailing number from `Socket.gethostname` (intended for Kubernetes pod ordinals). **`initialize` still sets `@machine_id = 0`**, so every id today is generated as if machine id were zero. Uniqueness across pods depends on clock + sequence + encoding, not host bits—document and fix before multi-node deploy.
